...
/**
 * Moderasyon kaydı oluşturur, veritabanına yazar ve ilgili özel log kanalına embed gönderir.
...
async function createModLog(client, guild, data) {
    ...
    // --- İlgili Özel Log Kanalını Belirle ---
    ...
    if (settings) {
        switch (type) {
            ...
            case 'tempban':
            case 'unban':
                logChannelId = settings.banLogChannel;
                break;
            case 'mute':
            case 'unmute':
                logChannelId = settings.muteLogChannel;
                break;
            case 'kick':
                logChannelId = settings.kickLogChannel;
                break;
            case 'warn':
                logChannelId = settings.warnLogChannel;
                break;
        }

        // Eğer eyleme özel log kanalı seçilmemişse genel mod log veya genel log kanalına fallback yap
        if (!logChannelId) {
            logChannelId = settings.modLogChannel || settings.logChannel;
        }
    }
